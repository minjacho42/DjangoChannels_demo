...
class Paddle:
    ...
    def check_collision(self, ball, direction):
        ...
        # 좌측 패들의 우측면 충돌 판정
        if direction == 'L' and ballL < padR and ballL > padL:
            # 우측면 충돌
            if ball.dx <= 0:
                # 우상단 -> 좌하단 접근 시
                if ball.dy <= 0 and ballB >= padT and ballB < self.y + self.height / 2:
                    ball.handle_collision('L', self.dy)
                # 우하단 -> 좌상단 접근 시
                elif ball.dy >= 0 and ballT <= padB and ballT > self.y + self.height / 2:
                    ball.handle_collision('L', self.dy)
                # 패들과 겹치는 경우
        ...
        # 우측 패들의 좌측면 충돌 판정
        elif direction == 'R' and ballR > padL and ballR < padR:
            # 좌측면 충돌
            if ball.dx >= 0:
                # 좌상단 -> 우하단 접근 시
                if ball.dy <= 0 and ballB >= padT and ballB < self.y + self.height / 2:
